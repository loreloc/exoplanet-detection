...
\section{Hyperparameters Tuning}
  ...
  \begin{algorithm}
    ...
    \begin{algorithmic}[1]
      \Procedure{Hyperband}{$T,L,S,\Omega,B,\eta,iter$}
        ...
        \For{$i \gets 0,...,iter$}
          ...
          \For{$j \gets 1,...,s$}
            ...
            \State $H \gets top_{n_{j}} \arg sort_{\omega} \{S(L(\omega \cup \{r_{j}\}), T) \mid \omega \in H\}$
          \EndFor
          \State $R \gets R \cup H$
        \EndFor
        \State \textbf{return} $\arg max_{\omega} \{S(L(\omega \cup \{B\}), T) \mid \omega \in R\}$
      \EndProcedure
    \end{algorithmic}
  \end{algorithm}
...
\section{Conclusion}
  ...
  \begin{table}
    ...
    \begin{tikzpicture}[scale=1.67]
      ...
      \coordinate[label={\textbf{862}}] (NN) at (1.5, 0.33);
    \end{tikzpicture}
